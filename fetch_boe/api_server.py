...
def thread_wrapper(ip: str, port: int, pool, stop_event: threading.Event):
    boa_key = f"{ip}:{port}"
    try:
        logger.info(f"[{boa_key}] Set SYSID_THISMAV → 21")
        _set_param(ip, port, "SYSID_THISMAV", 21)

    except Exception as e:
        logger.error(f"[{boa_key}] Impossibile settare SYSID 21: {e}")
        # Continuiamo comunque col thread di telemetria

    try:
        # Avvia la logica originale (apre il suo link interno)
...
def do_reposition(mav, lat, lon, alt):
    mav.mav.command_long_send(
        mav.target_system, mav.target_component,
        DO_REPOSITION, 0,
        0,                 # ground speed (unchanged)
        BITMASK_LLA,       # bitmask: use position
        0, 0,              # reserved, yaw
        float(lat), float(lon), float(alt))
    logger.info("[vaia] DO_REPOSITION inviato")
    ...
    try:
        ...
        while time.time() - start < VAI_A_TIMEOUT_SEC:
            ...
            if msg:
                ...
                logger.info(f"[vaia] Distanza corrente: {dist:.1f} m")
